...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_orcamento_1']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_orcamento_1']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_orcamento_1"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_link_contas_receber_idcliente = "<?php echo $this->form_encode_input($this->nmgp_dados_form['idcliente']); ?>";
function sc_btn_link_contas_receber()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_link_contas_receber(); }, 500);
      return;
    }
    sc_btn_link_contas_receber_ok();
}
function sc_btn_link_contas_receber_cancel()
{
}
function sc_btn_link_contas_receber_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_form_cliente_contas_receber_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "idcliente*scin' + var_btn_link_contas_receber_idcliente + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'form_cliente_contas_receber');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_orcamento_1']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("idcliente=disabled;data_venda=disabled;valor_total=disabled", "");
