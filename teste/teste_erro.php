...
$old_error_handler = set_error_handler("teste_scriptcase_error_handler");

function teste_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_teste->contr_teste->Apl_com_erro = true;
            if ($inicial_teste->contr_teste->NM_ajax_flag)
            {
                $inicial_teste->contr_teste->NM_ajax_info['errList']['geral_teste'][] = $msg;
                return;
            }
            teste_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function teste_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['teste'])  && '' != $_SESSION['scriptcase']['error_icon']['teste'])  ? $_SESSION['scriptcase']['error_icon']['teste']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['teste']) && '' != $_SESSION['scriptcase']['error_close']['teste']) ? $_SESSION['scriptcase']['error_close']['teste'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['teste']) && '' != $_SESSION['scriptcase']['error_span_title']['teste']) ? $_SESSION['scriptcase']['error_span_title']['teste'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['teste']) && '' != $_SESSION['scriptcase']['error_icon_title']['teste']) ? $_SESSION['scriptcase']['error_icon_title']['teste'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['teste']['error_buffer'] .= ob_get_flush();
}
