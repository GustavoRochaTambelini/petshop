...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_orcamento']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_orcamento']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_orcamento"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_cancelar_venda()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_cancelar_venda(); }, 500);
      return;
    }
    scJs_confirm("<?php echo html_entity_decode('Tem certeza que deseja CANCELAR essa venda?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_cancelar_venda_ok, sc_btn_cancelar_venda_cancel)
}
function sc_btn_cancelar_venda_cancel()
{
}
function sc_btn_cancelar_venda_ok()
{
    do_ajax_form_orcamento_event_scajaxbutton_cancelar_venda_onclick();
}
function sc_btn_concluir_venda()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_concluir_venda(); }, 500);
      return;
    }
    sc_btn_concluir_venda_ok();
}
function sc_btn_concluir_venda_cancel()
{
}
function sc_btn_concluir_venda_ok()
{
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_orcamento']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("idcliente=disabled", "");
