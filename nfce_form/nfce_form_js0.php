<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idorcamento" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idorcamento']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['nfce_form']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['nfce_form']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="nfce_form"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_btn_F9_codempresa = "<?php echo $this->form_encode_input($this->nmgp_dados_form['empresa_id']); ?>";
function sc_btn_btn_F9()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_F9(); }, 500);
      return;
    }
    scJs_confirm("<?php echo html_entity_decode('Tem certeza que deseja CANCELAR essa venda?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_btn_F9_ok, sc_btn_btn_F9_cancel);
}
function sc_btn_btn_F9_cancel()
{
}
function sc_btn_btn_F9_ok()
{
      nm_gp_submit('<?php echo $this->Ini->link_nfce_abertura_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "codempresa*scin' + var_btn_btn_F9_codempresa + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'nfce_abertura');
}
var Crtl_btn_btn_F7 = false;
function sc_btn_btn_F7()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_F7(); }, 500);
      return;
    }
    if (Crtl_btn_btn_F7) {return;}
    sc_btn_btn_F7_ok();
}
function sc_btn_btn_F7_cancel()
{
}
function sc_btn_btn_F7_ok()
{
    Crtl_btn_btn_F7 = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?btn_F7?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['nfce_form']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_nfce_form_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_nfce_form_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("qtde_item=disabled;sc_field_0=disabled;pra_pular=disabled", "I");
   $("#sc_btn_F9_top").css("background-color","red");
$("#sc_btn_F7_top").css("background-color","green");
$("#sc_btn_F4_top").css("background-color","orange");
$("#sc_btn_quantidade_top").css("background-color","blue");

$("#sc_btn_F9_top").css("font-size", "20px");
$("#sc_btn_F7_top").css("font-size", "20px");
$("#sc_btn_F4_top").css("font-size", "20px");
$("#sc_btn_quantidade_top").css("font-size", "20px");

$("#lin1_col1").css("font-size", "35px");
