...
function scHidePage(pageNo) {
	$("#id_form_cliente_etapas_mob_form" + pageNo).hide();
}
...
function scJQCalendarAdd(iSeqRow) {
  $("#id_sc_field_data_nascimento" + iSeqRow).datepicker({
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_cliente_etapas_mob_validate_data_nascimento(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_cliente_etapas_mob')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
...
var wizardActualStep = <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_cliente_etapas_mob']['form_wizard']['actual_step']; ?>;
var wizardTotalSteps = <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_cliente_etapas_mob']['form_wizard']['total_steps']; ?>;
...
function scJQWizardValidateStep(stepGoTo)
{
    if (0 == wizardActualStep) {
        do_ajax_form_cliente_etapas_mob_submit_page_0(stepGoTo);
    }
    if (1 == wizardActualStep) {
        do_ajax_form_cliente_etapas_mob_submit_page_1(stepGoTo);
    }
    if (2 == wizardActualStep) {
        do_ajax_form_cliente_etapas_mob_submit_page_2(stepGoTo);
    }
    if (3 == wizardActualStep) {
        do_ajax_form_cliente_etapas_mob_submit_page_3(stepGoTo);
    }
}
...
{
    $("#form_cliente_etapas_mob_form" + stepNo).css({
        "width": "1px",
        "height": "0",
        "display": "none",
        "overflow": "scroll",
