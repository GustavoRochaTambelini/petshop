<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idconta_corrente" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idconta_corrente']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_conta_corrente_detalhe']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_conta_corrente_detalhe']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_conta_corrente_detalhe"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_btn_gravar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_gravar(); }, 500);
      return;
    }
    sc_btn_btn_gravar_ok();
}
function sc_btn_btn_gravar_cancel()
{
}
function sc_btn_btn_gravar_ok()
{
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_conta_corrente_detalhe']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_conta_corrente_detalhe_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_conta_corrente_detalhe_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
