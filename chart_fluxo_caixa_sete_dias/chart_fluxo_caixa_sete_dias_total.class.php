...
class chart_fluxo_caixa_sete_dias_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_fluxo_caixa_sete_dias']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_fluxo_caixa_sete_dias']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['tot_geral'] = array() ;  
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_data_sc_free_group_by($data, $Where_qb, $Cmp_Name) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['where_pesq'])) 
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("pt_br");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('data' => "data");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_fluxo_caixa_sete_dias']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
