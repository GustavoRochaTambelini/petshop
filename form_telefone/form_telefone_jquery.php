...
function scEventControl_init(iSeqRow) {
  scEventControl_data["idtelefone_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["numero_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["tipo_contato_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["idtelefone_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idtelefone_" + iSeqRow]["change"]) {
    return true;
  }
  ...
  if (scEventControl_data["tipo_contato_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["tipo_contato_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scEventControl_onFocus(oField, iSeq) {
  ...
  if ("tipo_contato_" + iSeq == fieldName) {
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_idtelefone_' + iSeqRow).bind('blur', function() { sc_form_telefone_idtelefone__onblur(this, iSeqRow) })
                                         .bind('change', function() { sc_form_telefone_idtelefone__onchange(this, iSeqRow) })
                                         .bind('focus', function() { sc_form_telefone_idtelefone__onfocus(this, iSeqRow) });
  $('#id_sc_field_idcliente_' + iSeqRow).bind('change', function() { sc_form_telefone_idcliente__onchange(this, iSeqRow) });
  $('#id_sc_field_numero_' + iSeqRow).bind('blur', function() { sc_form_telefone_numero__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_telefone_numero__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_telefone_numero__onfocus(this, iSeqRow) });
  $('#id_sc_field_tipo_contato_' + iSeqRow).bind('blur', function() { sc_form_telefone_tipo_contato__onblur(this, iSeqRow) })
                                           .bind('change', function() { sc_form_telefone_tipo_contato__onchange(this, iSeqRow) })
                                           .bind('focus', function() { sc_form_telefone_tipo_contato__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_telefone_idtelefone__onblur(oThis, iSeqRow) {
  do_ajax_form_telefone_validate_idtelefone_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_telefone_idtelefone__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_telefone_idtelefone__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_telefone_idcliente__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_telefone_numero__onblur(oThis, iSeqRow) {
  do_ajax_form_telefone_validate_numero_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_telefone_numero__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_telefone_numero__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_telefone_tipo_contato__onblur(oThis, iSeqRow) {
  do_ajax_form_telefone_validate_tipo_contato_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_telefone_tipo_contato__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_telefone_tipo_contato__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("idtelefone_", "", status);
	displayChange_field("numero_", "", status);
	displayChange_field("tipo_contato_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_idtelefone_(row, status);
	displayChange_field_numero_(row, status);
	displayChange_field_tipo_contato_(row, status);
}

function displayChange_field(field, row, status) {
	if ("idtelefone_" == field) {
		displayChange_field_idtelefone_(row, status);
	}
	if ("numero_" == field) {
		displayChange_field_numero_(row, status);
	}
	if ("tipo_contato_" == field) {
		displayChange_field_tipo_contato_(row, status);
	}
}

function displayChange_field_idtelefone_(row, status) {
    var fieldId;
}
...
function displayChange_field_tipo_contato_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_telefone_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_telefone')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
...
function scJQSelect2Add(seqRow, specificField) {
  if (null == specificField || "idcliente_" == specificField) {
    scJQSelect2Add_idcliente_(seqRow);
  }
} // scJQSelect2Add

function scJQSelect2Add_idcliente_(seqRow) {
  var elemSelector = "all" == seqRow ? ".css_idcliente__obj" : "#id_sc_field_idcliente_" + seqRow;
  $(elemSelector).select2(
    {
      containerCssClass: 'css_idcliente__obj',
      dropdownCssClass: 'css_idcliente__obj',
      language: {
        noResults: function() {
...
function scJQElementsAdd(iLine) {
  ...
  setTimeout(function () { if ('function' == typeof displayChange_field_idcliente_) { displayChange_field_idcliente_(iLine, "on"); } }, 150);
} // scJQElementsAdd
