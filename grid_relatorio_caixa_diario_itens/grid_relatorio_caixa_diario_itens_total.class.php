...
class grid_relatorio_caixa_diario_itens_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_relatorio_caixa_diario_itens']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_relatorio_caixa_diario_itens']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $itens_orcamento_idproduto, $contas_receber_idforma_pagamento, $contas_receber_idcliente;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(itens_orcamento.valor_total), sum(contas_receber.valor_recebido) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(itens_orcamento.valor_total), sum(contas_receber.valor_recebido) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['contr_total_geral'] = "OK";
   } 

   //-----  contas_receber_idcliente
   function quebra_contas_receber_idcliente_sc_free_group_by($contas_receber_idcliente, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $itens_orcamento_idproduto, $contas_receber_idforma_pagamento, $contas_receber_idcliente;
      $tot_contas_receber_idcliente = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(itens_orcamento.valor_total), sum(contas_receber.valor_recebido) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_relatorio_caixa_diario_itens']['where_pesq']; 
      } 
      ...
      $tot_contas_receber_idcliente[0] = NM_encode_input(sc_strip_script($contas_receber_idcliente)) ; 
      $tot_contas_receber_idcliente[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_contas_receber_idcliente[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_contas_receber_idcliente[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_contas_receber_idcliente;
   } 
