<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idcaixa_diario" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idcaixa_diario']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_caixa_diario_vendas']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_caixa_diario_vendas']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_caixa_diario_vendas"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_rel_orcamento_diario()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_rel_orcamento_diario(); }, 500);
      return;
    }
    sc_btn_rel_orcamento_diario_ok();
}
function sc_btn_rel_orcamento_diario_cancel()
{
}
function sc_btn_rel_orcamento_diario_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_orcamento_diario_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', 'inicio', '_self', '0', '0', 'grid_orcamento_diario');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_caixa_diario_vendas']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_caixa_diario_vendas_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_caixa_diario_vendas_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("saldo=disabled", "");
