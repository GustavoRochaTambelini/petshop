...
function scEventControl_init(iSeqRow) {
  scEventControl_data["idpacote_itens_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["idproduto_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["quantidade_produto_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["idpacote_itens_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idpacote_itens_" + iSeqRow]["change"]) {
    return true;
  }
  ...
  if (scEventControl_data["quantidade_produto_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["quantidade_produto_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_idpacote_itens_' + iSeqRow).bind('blur', function() { sc_form_pacote_itens_idpacote_itens__onblur(this, iSeqRow) })
                                             .bind('change', function() { sc_form_pacote_itens_idpacote_itens__onchange(this, iSeqRow) })
                                             .bind('focus', function() { sc_form_pacote_itens_idpacote_itens__onfocus(this, iSeqRow) });
  $('#id_sc_field_idpacote_' + iSeqRow).bind('change', function() { sc_form_pacote_itens_idpacote__onchange(this, iSeqRow) });
  $('#id_sc_field_idproduto_' + iSeqRow).bind('blur', function() { sc_form_pacote_itens_idproduto__onblur(this, iSeqRow) })
                                        .bind('change', function() { sc_form_pacote_itens_idproduto__onchange(this, iSeqRow) })
                                        .bind('focus', function() { sc_form_pacote_itens_idproduto__onfocus(this, iSeqRow) });
  $('#id_sc_field_quantidade_produto_' + iSeqRow).bind('blur', function() { sc_form_pacote_itens_quantidade_produto__onblur(this, iSeqRow) })
                                                 .bind('change', function() { sc_form_pacote_itens_quantidade_produto__onchange(this, iSeqRow) })
                                                 .bind('focus', function() { sc_form_pacote_itens_quantidade_produto__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_pacote_itens_idpacote_itens__onblur(oThis, iSeqRow) {
  do_ajax_form_pacote_itens_validate_idpacote_itens_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_pacote_itens_idpacote_itens__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_pacote_itens_idpacote_itens__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_pacote_itens_idpacote__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_pacote_itens_idproduto__onblur(oThis, iSeqRow) {
  do_ajax_form_pacote_itens_validate_idproduto_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_pacote_itens_idproduto__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_pacote_itens_idproduto__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_pacote_itens_quantidade_produto__onblur(oThis, iSeqRow) {
  do_ajax_form_pacote_itens_validate_quantidade_produto_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_pacote_itens_quantidade_produto__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_pacote_itens_quantidade_produto__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("idpacote_itens_", "", status);
	displayChange_field("idproduto_", "", status);
	displayChange_field("quantidade_produto_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_idpacote_itens_(row, status);
	displayChange_field_idproduto_(row, status);
	displayChange_field_quantidade_produto_(row, status);
}

function displayChange_field(field, row, status) {
	if ("idpacote_itens_" == field) {
		displayChange_field_idpacote_itens_(row, status);
	}
	if ("idproduto_" == field) {
		displayChange_field_idproduto_(row, status);
	}
	if ("quantidade_produto_" == field) {
		displayChange_field_quantidade_produto_(row, status);
	}
}

function displayChange_field_idpacote_itens_(row, status) {
    var fieldId;
}
...
function displayChange_field_quantidade_produto_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_pacote_itens_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_pacote_itens')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
