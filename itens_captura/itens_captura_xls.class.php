...
class itens_captura_xls
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "itens_captura/itens_captura_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              require_once($this->Ini->path_aplicacao . "itens_captura_res_xls.class.php");
              $this->Res_xls = new itens_captura_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_itens_captura.zip";
          $this->Arquivo   .= "_itens_captura" . $this->Xls_tp;
          $this->Tit_doc    = "itens_captura" . $this->Xls_tp;
          $this->Tit_zip    = "itens_captura.zip";
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['field_order'] as $Cada_cmp)
      {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name'] .= $this->Xls_tp;
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
     ...
     $nm_houve_quebra = "N";
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   function NM_export_codigo_barras()
   {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->codigo_barras, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->codigo_barras, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- descricao
   ...
   function NM_export_und_comercial()
   {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->und_comercial, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->und_comercial, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- valor_venda
   ...
   function NM_export_imagem()
   {
         ...
         $in_imagem = $this->Ini->root . $this->Ini->path_imag_temp . "/sc_imagem_" . $_SESSION['scriptcase']['sc_num_img'] . session_id() . ".jpg";
         $_SESSION['scriptcase']['sc_num_img']++;
         $arq_imagem = fopen($in_imagem, "w");
         if (is_string($this->imagem) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
         { 
             $nm_tmp = nm_conv_img_access(substr($this->imagem, 0, 12));
             if (substr($nm_tmp, 0, 4) == "*nm*")
             { 
                 $this->imagem = nm_conv_img_access($this->imagem);
             } 
         } 
         if (is_string($this->imagem) && substr($this->imagem, 0, 4) == "*nm*")
         { 
             $this->imagem = substr($this->imagem, 4);
             $this->imagem = base64_decode($this->imagem);
         } 
         $img_pos_bm = (is_string($this->imagem)) ? strpos($this->imagem, "BM") : false;
         if (!$img_pos_bm === FALSE && $img_pos_bm == 78)
         { 
             $this->imagem = substr($this->imagem, $img_pos_bm);
         } 
         fwrite($arq_imagem, (string)$this->imagem);
         fclose($arq_imagem);
         $md5_imagem  = md5($this->imagem);
         if (is_file($in_imagem) && !$this->Ini->Gd_missing)
         { 
             $out_imagem = $this->Ini->path_imag_temp . "/sc_imagem_" . $md5_imagem . ".jpg";
             $tmp_imagem = fopen($in_imagem, "r");
             $reg_imagem = fread($tmp_imagem, filesize($in_imagem));
             fclose($tmp_imagem);
             $arq_imagem = fopen($this->Ini->root . $out_imagem, "w");
             fwrite($arq_imagem, $reg_imagem);
             fclose($arq_imagem);
             $in_imagem = $this->Ini->root . $out_imagem;
             $sc_obj_img = new nm_trata_img($in_imagem);
             $sc_obj_img->setWidth(50);
             $sc_obj_img->setHeight(50);
             $sc_obj_img->setManterAspecto(true);
             $sc_obj_img->createImg($this->Ini->root . $out_imagem);
         } 
         if (is_file($in_imagem))
         { 
             $sc_obj_img = new nm_trata_img($in_imagem);
   ...
   //----- codigo_barras
   function NM_sub_cons_codigo_barras()
   {
         $this->codigo_barras = html_entity_decode($this->codigo_barras, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->codigo_barras = strip_tags($this->codigo_barras);
         $this->codigo_barras = NM_charset_to_utf8($this->codigo_barras);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->codigo_barras;
   ...
   //----- und_comercial
   function NM_sub_cons_und_comercial()
   {
         $this->und_comercial = html_entity_decode($this->und_comercial, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->und_comercial = strip_tags($this->und_comercial);
         $this->und_comercial = NM_charset_to_utf8($this->und_comercial);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->und_comercial;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
   ...
   function NM_sub_cons_imagem()
   {
         ...
         if (is_file($in_imagem))
         { 
             $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $in_imagem;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['nolabel'])
       {
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_file'] = $this->Xls_f;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - erp_itens :: Excel</TITLE>
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['itens_captura']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
