...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>form_cliente/form_cliente_mob_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
else
{
    ...
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_cliente_mob']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_cliente_mob']['link_info']['remove_margin']) {
        $remove_margin = 'margin: 0; ';
    }
    if ('' != $remove_margin && isset($str_iframe_body) && '' != $str_iframe_body) {
        $str_iframe_body = '';
    }
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_cliente_mob']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_cliente_mob']['link_info']['remove_border']) {
...
<?php
    ...
    if ((!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_cliente_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_cliente_mob']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
...
<script type="text/javascript">
var pag_ativa = "form_cliente_mob_form0";
</script>
<ul class="scTabLine sc-ui-page-tab-line">
<?php
    $this->tabCssClass = array(
        'form_cliente_mob_form0' => array(
            'title' => "Informações Cliente",
            'class' => empty($nmgp_num_form) || $nmgp_num_form == "form_cliente_mob_form0" ? "scTabActive" : "scTabInactive",
        ),
        'form_cliente_mob_form1' => array(
            'title' => "Contato",
            'class' => $nmgp_num_form == "form_cliente_mob_form1" ? "scTabActive" : "scTabInactive",
        ),
        'form_cliente_mob_form2' => array(
            'title' => "Outros responsáveis",
            'class' => $nmgp_num_form == "form_cliente_mob_form2" ? "scTabActive" : "scTabInactive",
        ),
        'form_cliente_mob_form3' => array(
            'title' => "Pet`s",
            'class' => $nmgp_num_form == "form_cliente_mob_form3" ? "scTabActive" : "scTabInactive",
        ),
    );
    if (!empty($this->Ini->nm_hidden_pages)) {
        foreach ($this->Ini->nm_hidden_pages as $pageName => $pageStatus) {
            if ('Informações Cliente' == $pageName && 'off' == $pageStatus) {
                $this->tabCssClass['form_cliente_mob_form0']['class'] = 'scTabInactive';
            }
            if ('Contato' == $pageName && 'off' == $pageStatus) {
                $this->tabCssClass['form_cliente_mob_form1']['class'] = 'scTabInactive';
            }
            if ('Outros responsáveis' == $pageName && 'off' == $pageStatus) {
                $this->tabCssClass['form_cliente_mob_form2']['class'] = 'scTabInactive';
            }
            if ('Pet`s' == $pageName && 'off' == $pageStatus) {
                $this->tabCssClass['form_cliente_mob_form3']['class'] = 'scTabInactive';
            }
        }
...
?>
<?php
    $css_celula = $this->tabCssClass["form_cliente_mob_form0"]['class'];
?>
   <li id="id_form_cliente_mob_form0" class="<?php echo $css_celula; ?> sc-form-page sc-tab-click" data-tab-name="form_cliente_mob_form0">
     Informações Cliente
   </li>
<?php
    $css_celula = $this->tabCssClass["form_cliente_mob_form1"]['class'];
?>
   <li id="id_form_cliente_mob_form1" class="<?php echo $css_celula; ?> sc-form-page sc-tab-click" data-tab-name="form_cliente_mob_form1">
     Contato
   </li>
<?php
    $css_celula = $this->tabCssClass["form_cliente_mob_form2"]['class'];
?>
   <li id="id_form_cliente_mob_form2" class="<?php echo $css_celula; ?> sc-form-page sc-tab-click" data-tab-name="form_cliente_mob_form2">
     Outros responsáveis
   </li>
<?php
    $css_celula = $this->tabCssClass["form_cliente_mob_form3"]['class'];
?>
   <li id="id_form_cliente_mob_form3" class="<?php echo $css_celula; ?> sc-form-page sc-tab-click" data-tab-name="form_cliente_mob_form3">
     Pet`s
   </li>
</ul>
...
<div id="form_cliente_mob_form0" style='display: none; width: 1px; height: 0px; overflow: scroll'>
...
<span id="id_read_on_cep" class="sc-ui-readonly-cep css_cep_line" style="<?php echo $sStyleReadLab_cep; ?>"><?php echo $this->form_format_readonly("cep", $this->form_encode_input($this->cep)); ?></span><span id="id_read_off_cep" class="css_read_off_cep<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_cep; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=8"; } ?> maxlength=29 alt="{datatype: 'mask', maxLength: 20, allowedChars: '<?php echo $this->allowedCharsCharset("0123456789") ?>', lettersCase: '', maskList: '99999-999', enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
...
else
{
   ...
   while (!empty($todo[$x])) 
   {
          ...
          elseif (trim($this->uf) === trim($cadaselect[1])) { $uf_look .= $cadaselect[0]; } 
          $x++; 
   }
