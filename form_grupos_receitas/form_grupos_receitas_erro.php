...
$old_error_handler = set_error_handler("form_grupos_receitas_scriptcase_error_handler");

function form_grupos_receitas_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['form_grupos_receitas']['contr_erro']) && $_SESSION['scriptcase']['form_grupos_receitas']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_grupos_receitas->contr_form_grupos_receitas->Apl_com_erro = true;
            if ($inicial_form_grupos_receitas->contr_form_grupos_receitas->NM_ajax_flag)
            {
                $inicial_form_grupos_receitas->contr_form_grupos_receitas->NM_ajax_info['errList']['geral_form_grupos_receitas'][] = $msg;
                return;
            }
            form_grupos_receitas_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_grupos_receitas_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_grupos_receitas'])  && '' != $_SESSION['scriptcase']['error_icon']['form_grupos_receitas'])  ? $_SESSION['scriptcase']['error_icon']['form_grupos_receitas']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_grupos_receitas']) && '' != $_SESSION['scriptcase']['error_close']['form_grupos_receitas']) ? $_SESSION['scriptcase']['error_close']['form_grupos_receitas'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_grupos_receitas']) && '' != $_SESSION['scriptcase']['error_span_title']['form_grupos_receitas']) ? $_SESSION['scriptcase']['error_span_title']['form_grupos_receitas'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_grupos_receitas']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_grupos_receitas']) ? $_SESSION['scriptcase']['error_icon_title']['form_grupos_receitas'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_grupos_receitas']['error_buffer'] .= ob_get_flush();
}
