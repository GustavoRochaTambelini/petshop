...
<?php
   ...
   $Ord_Cmp = new grid_contas_pagar_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_contas_pagar_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select_orig'] = array();
   }
   ...
   $tab_labels["cpf_cnpj"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["cpf_cnpj"])) ? $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["cpf_cnpj"] : "CPF/CNPJ";
   $tab_ger_campos['nome_fantasia'] = "on";
   $tab_def_campos['nome_fantasia'] = "nome_fantasia";
   $tab_labels["nome_fantasia"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["nome_fantasia"])) ? $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["nome_fantasia"] : "NOME FANTASIA";
   $tab_ger_campos['valor_a_pagar'] = "on";
   $tab_def_campos['valor_a_pagar'] = "valor_a_pagar";
   $tab_labels["valor_a_pagar"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["valor_a_pagar"])) ? $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["valor_a_pagar"] : "VALOR A PAGAR";
   $tab_ger_campos['valor_pago'] = "on";
   $tab_def_campos['valor_pago'] = "valor_pago";
   $tab_labels["valor_pago"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["valor_pago"])) ? $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["valor_pago"] : "VALOR PAGO";
   $tab_ger_campos['juros'] = "on";
   $tab_def_campos['juros'] = "juros";
   $tab_labels["juros"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["juros"])) ? $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['labels']["juros"] : "JUROS";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_contas_pagar_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_contas_pagar']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
